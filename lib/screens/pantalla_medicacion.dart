  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Medicación', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Editar',
            onPressed: () {
              // TODO: navegar a pantalla de edición de medicación
            },
          ),
        ],
      ),
      body: perfil.medicacion.isEmpty
          ? const _SinDatos(mensaje: 'No se registró medicación.')
          : ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount: perfil.medicacion.length,
              itemBuilder: (context, i) => SeccionCard(texto: perfil.medicacion[i]),
            ),
    );
  }
}